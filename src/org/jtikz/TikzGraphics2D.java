...
public class TikzGraphics2D extends AbstractGraphicsInterface {
        ...
        while (!i.isDone()) {
            ...
            switch (type) {
                ...
                case PathIterator.SEG_MOVETO:
                    tikz += " (" + format.format(s[0]) + "pt, " + format.format(s[1]) + "pt)";
                    break;
                case PathIterator.SEG_QUADTO:
                    // TODO: Implement this later!
                    break;
                case PathIterator.SEG_CUBICTO:
                    tikz += " .. (" + format.format(s[0]) + "pt, " + format.format(s[1]) + "pt) and (" + format.format(s[2]) + "pt, " + format.format(s[3]) + "pt) .. (" + format.format(s[4]) + "pt, " + format.format(s[5]) + "pt)";
                    break;
    ...
    protected void handleLine(double x1, double y1, double x2, double y2) {
        addCommand("\\draw" + handleOptions() + " (" + format.format(x1) + "pt, " + format.format(y1) + "pt) -- (" + format.format(x2) + "pt, " + format.format(y2) + "pt);");
    }
